...
| `E_BindOverBound`     | the graph's one type writer was asked to bind a cell that already holds a type — a rewrite that would DISCARD a proof where a unification reads it. A report on the medium, never on the program: it is how `!5`, `5 && true` and `if 5 { … }` all checked clean (the operand's cell written with `Bool`, 2026-09-24). Born, measured at 5,881 on the wheel's own compile, and ARMED the same day: operators and `if` unify their operands, a let annotation types the BINDER's cell, a parameter default is judged once at its declaration and referenced by every call that omits the field (§«Default parameter values» — the frontier gate found it re-inferred per call), and the ownership pass writes grades through its own op (`graph_grade`, which refuses a write that would change anything but a param's resolved slot). The writer refuses; fixtures `tests/syntax/not-over-int`, `and-over-int`, `if-over-int`, `not-over-partial` hold the four shapes as `E_TypeMismatch` refusals, and `default-judged-once` holds the fifth as a run | `MaybeIncorrect` | none for the developer — the compiler's own writer is wrong; report it |
...
| `T_EqTypeUnprovable` | a comparison (`==`, `!=`, `<`, …) whose operand type is still a variable at emit — no structure to read, so it falls to a one-word compare: value-equality for a word, an ADDRESS lie for anything else. The diagnostic carries the operator and the operand's live type at the operand's span. BORN 2026-09-18 as narration: a handler arm over quantified op parameters answered `"ab" != "ab"` (`tests/frontier/mn-eq-in-arm-pointer.mn`, declared red), and the wheel carries such compares itself — the trap form was refuted by the wheel dying on its own compile. Pre-arm; the `Said(UnprovableComparison)` bound in src/board.mn is the count's one home and the countdown (`mentl verify` reads the compile's own tally), `T_FieldOffsetUnprovable`'s sibling on the same ladder. IT SEES ONE ALTITUDE ONLY, and the limit is measured rather than suspected: a polymorphic sum's payload compared by address inside a GENERATED leaf, where no authored comparison stands for this diagnostic to attach to, so the count held unchanged across that defect's whole discovery. That one is closed (`Hβ.eq.polymorphic-sum-payload-is-pointer-eq`); the altitude limit is not, and the generated leaf's own refusal is `Hβ.emit.generated-leaf-swallows-an-unresolved-payload` | `MaybeIncorrect` | prove the operand — give the call site a shape the twin can key on, or name the type at its Intent Boundary |
